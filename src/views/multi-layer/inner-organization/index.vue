  ...
  <el-container>
    ...
    <el-main>
      <el-tabs v-model="activeName">
        <el-tab-pane label="共生体展示" name="first">
          <!--<el-row>-->
          <!--<el-col :span="4">-->
          <!--【筛选条件】原子服务间关系：-->
          ...
          <el-row>
            <el-table :data="sspNodeShows">
              ...
              <el-table-column
                ...
                label="出现频率"
              />
              <el-table-column label="内部关系">
                <template slot-scope="scope">
                  <el-button @click="showRel(scope.row.id)">查看内部关系</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-row>
        </el-tab-pane>
        <el-tab-pane label="共生体间关系" name="second">
          <el-row>
            <el-col :span="4">
              【筛选条件】共生体间关系：
              <el-select v-model="listQuery.sspR" multiple placeholder="请选择">
                <el-option
                  v-for="item in ['整体局部','功能覆盖','功能互补']"
                  :key="item"
                  :label="item"
                  :value="item"
                />
        ...
        <el-tab-pane label="组织内原子服务" name="third">
    ...
    <el-dialog :visible.sync="isShown">
      <div>
        <h3>共生体内部关系</h3>
        <el-table :data="choosed_service_edges">
          <el-table-column width="50">
            <template slot-scope="scope">
              <span>{{ scope.$index + 1 }}</span>
            </template>
          ...
          >
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ id2service(scope.row.source).serviceName }}</span>
            </template>
          </el-table-column>
          <el-table-column
            label="tail"
          >
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ id2service(scope.row.target).serviceName }}</span>
            </template>
          </el-table-column>
          <el-table-column
            label="关系"
...
export default {
  ...
  computed: {
    sspNodeShows: function() {
      return this.sspGraph.nodes.filter(nd => nd.provider == this.listQuery.provider)
    },
    sspEdgeShows: function() {
      const edges = this.sspGraph.edges.filter(ed =>
        this.sspGraph.nodes.filter(nd => nd.id == ed.source)
          .every(nd => nd.provider == this.listQuery.provider))
  ...
  methods: {
    ...
    showRel(sspId) {
      const ssp = this.sspGraph.nodes.filter(sp => sp.id == sspId)
      if (ssp.length > 0) {
        const choosed_services = ssp[0].services
        this.choosed_service_edges = this.serviceEdges.filter(edgs =>
          choosed_services.indexOf(parseInt(edgs.source)) >= 0 && choosed_services.indexOf(parseInt(edgs.target)) >= 0)

        console.info(ssp, choosed_services, this.choosed_service_edges)
        this.isShown = true
      }
    },
    alert(text) {
